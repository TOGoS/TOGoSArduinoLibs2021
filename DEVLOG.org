...
| D1    | GPIO5  | Ok           | Ok            | Often used as SCL (I2C), default (active high) pin for [[https://www.wemos.cc/en/latest/d1_mini_shield/relay.html][WeMos relay shield]] |
...
and snapshotted as [[http://picture-files.nuke24.net/uri-res/raw/urn:bitprint:LQJEUHRR2MHJTYJTHHZ4RYAWBDZR2ACS.C7LMOJ47BI3WH7BOXCUQ7U3DNQPMKAKGFXSC7GQ/EnvironmentalSensor2021Wiring-v0.1.pdf][EnvironmentalSensor2021Wiring-v0.1.pdf]] (and [[http://picture-files.nuke24.net/uri-res/raw/urn:bitprint:HTS6UGQ2KCFDQI4DRCWABZNHKWQVXYUJ.G2YUMX4S2MVPZ3ZX74P5CLOS2KWRKAWQADTTLGY/20250712-EnvironmentalSensor2021Wiring-v0.1-5bit.png][20250712-EnvironmentalSensor2021Wiring-v0.1-5bit.png]]).
...
Let's call the current assembly WSITEM-201770.

** 2025-07-24 - Thoughts about polishing

Might be useful to be able to send ~#HELO/get~ packets to the thing
and have it respond -- this would allow:
- Getting updates at a different rate than the broadcast packets
- Getting data on e.g. Windows machines that can't receive the broadcasts!

Screen issues;
- "ES2021" is "vS2021" (I think because the "0.0.13-dev" wraps around!)
- Might want to periodically reinitialize screen so that it continues to work
  when dis/reconnected
- Bottom row is garbage!  Clear that out!

[[https://www.nuke24.net/docs/2025/WSITEM-201770.html][WSITEM-201770]]'s screen is kinda dim.  I wondered if giving 5V instead of 3.3V would help,
since [[https://www.ebay.com/itm/294691149010][product page of most recently bought ones]] says "Voltage: 3V ~ 5V DC".
Tested with another screen that it is at least allowed -- nothing blows up,
but this new screen is bright either way.

Finally tested WSITEM-201770's current screen.  No brighter at 5V either.

Which means, unless I'm forgetting something about these WeMos D1 minis,
there's nothing on the board that requires more than 3.3V, and you could
power the whole thing from the I2C/SHT20 port if you wanted.
...
- [X] Buy more little SSD1306 OLED displays
  - WSORDER-300221
- [X] Prototype a panel for the display (maybe with the button under it?)
- [X] I think this case isn't going to be quite deep enough.  Print a deeper one!  9" long!
  ...or pring a couple of 'spacer rails' or something.
  Maybe that's how these things could be assembled.
  - 1" rails should do the job (see WSITEM-3424 p53, 'way 3')
- [X] Indicate on screen when button pressed
- [X] Make alternate screen mode to show T/H from all four sensors at once
- [X] Cut a version, maybe call this v0.0.12.
- [X] Print one giant panel so this can all be wired up easily
  - Well, a small one; WSITEM-201875 + innards + lid being an example, using part set p2124,
    i.e. Set of p2088, p2089, p2094

** [2025-10-18]

I wired the thing up and got it to work, receiving a packet using `nc -u -l :: 16378` once.

#+begin_example
#HELO

app-name EnvironmentalSensor2021
app-version 0.0.13-dev
mac 60:01:94:70:2D:0B
clock 593012
touch-button/pressed 0
touch-button/last-pressed 0
temhum0/last-read 592339
temhum0/connected true
temhum0/temperature/c 23.45
temhum0/temperature/f 74.22
temhum0/humidity/percent 59.46
temhum1/last-read 592339
temhum1/connected false
temhum2/last-read 592778
temhum2/connected false
temhum3/last-read 593012
temhum3/connected false
#+end_example

Commentary by me:

- Waiting for it to automatically update after being powered on...
- [19:09]: Waiting!
- [19:13] Still waiting.
- Okay now I am running temhum0/read at 19:22, to try to force it to update.  Thsi is redick.
- Hmm, seems like it died or something?
- Restarted at [19:24]
- Still didn't send anything.

Potential sources of not seeing more data:
- 'Nc seems to only spit out the first packet received' (something I discovered earlier)
- ES2021 only occasionally sending new HELO packets (how often?)
and

It would be nice to report when ~#status~
- [X] Exact version of the software
  - Well, v0.0.14 does provide a standard-ish way to indicate config-hash,
    which in combination with the app version and the assumption that ~config.h~ is written
    a certain way, may be good enough.
- [\] HELO port
- [\] When the last HELP packet was sent, how long until the next one
  - Note that ~verbosity/set 60~ will also reveal this!

[2025-10-18T19:33]: Would I have more luck with ~HELODemo/UDPListen.ts~?

#+begin_src 
deno run --allow-net --unstable-net UDPListen.ts [::]:16378
#+end_src

HMM NOPE, NOT SEEING 'EM, even though I am being told over serial that
it is connected to WiFi and broadcasting HELO packets.

Oh, maybe it was *not* connected to WiFi for some reason.
Maybe it forgot the network?

After running ~wifi/connect~ with my SSID and password,
that invocation of ~UDPListen.ts~ is now showing stuff showing up.
Stuff like

#+begin_example
Received 431 bytes from {"hostname":"fe80::6201:94ff:fe70:2d0b","port":55317,"transport":"udp"}: text:
  #HELO
  ...
  touch-button/pressed 0
  touch-button/last-pressed 0
  temhum0/last-read 955681
  temhum0/connected true
  temhum0/temperature/c 24.38
  temhum0/temperature/f 75.88
  temhum0/humidity/percent 55.94
  temhum1/last-read 955953
  temhum1/connected false
  temhum2/last-read 956189
  temhum2/connected false
  temhum3/last-read 956423
  temhum3/connected false
#+end_example

[2025-10-18T19:40]: What if I power it off and back on??

#+begin_example
# Number of wifi networks to which I'll attempt to auto-connect;
# (if zero, I may attempt to just begin(); maybe that should be a setting):
wifi-config/wifi-network-count 0
net-config/use-static-ip4 true
wifi/mac-address 60:01:94:70:2D:0B
wifi/status-code 0
wifi/ssid 
wifi/connected false
wifi/auto-connect true
wifi/auto-reconnect true
net/ipv6-enabled true
net/addr IF='st' IPv6=1 local=1 hostname='ESP-702D0B' addr= fe80::6201:94ff:fe70:2d0b
mqtt/server/host 
mqtt/server/port 0
mqtt/connected false
screen0/row-count 8
screen0/column-count 128
# status: Ok
#+end_example

Hmm.  Maybe that auto-connect-to-last-connected-network
doesn't work as well as I thought, and I'll need to make
a branch of sorts with hard-coded networks after all.

Note: With the flat lid, this thing fits perfectly (thickness-wise,
anyway, and discounting the rubber band) into one of my phone holders.

** 2025-11-01 - RelayTimer updates

*** DONE Make timer durations configurable
*** DONE Update configuration system

- Should need minimal and obvious changes to create instance from trunk
- Include config.h in repo
- Include 'empty' ~secrets-(whatever the hash is).h~ file, reference from config.h
- .gitignore secrets files except for the empty one
- Switch to 'real version number' for core (i.e. RelayTimer.ino)
  - Can start at v3.0 maybe, since there is a ~apps/instances/2025/20250310-plant-light-timer/v2.1~ tag.

*** DONE Connect to network and broadcast status in HELO packets, similar to ES2021

See my personal instance configuration, ~x-git-object:b9e021efa8bfdad69c10b9bebca749306a0fcce2#RelayTimer/RelayTimer.ino~.

One little improvement I am thinking could be made is to have standard
(across all apps) names for the hardcoded WiFi network constants so
that a single ~secrets-....h~ (or possibly different ones for
different features; ~wifi-secrets-...h~, ~mqtt-secrets...h~, etc)
could be used across the apps without having to rename stuff
and make new header files.

*** DONE Make that improvement

My updated personal configuration is now ~x-git-object:010962aaa7415c4b3ffee48ebe0aecad59bb21fa#RelayTimer/RelayTimer.ino~.
